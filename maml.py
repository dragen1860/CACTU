...
class MAML:
    def __init__(self, dim_input=1, dim_output_train=1, dim_output_val=1, test_num_updates=5):
        ...
        if FLAGS.on_encodings:
            ...
            if FLAGS.conv:
                ...
                self.forward = self.forward_fc
                self.construct_weights = self.construct_fc_weights
        if FLAGS.datasource == 'mnist' or FLAGS.datasource == 'omniglot':
            self.channels = 1
        else:
            self.channels = 3
        self.img_size = int(np.sqrt(self.dim_input / self.channels))
        if FLAGS.datasource not in ['mnist', 'omniglot', 'miniimagenet', 'celeba']:
    ...
    def construct_model(self, input_tensors=None, prefix='metatrain_'):
                    ...
                    for j in range(num_updates):
                        task_accuraciesb.append(
                            tf.contrib.metrics.accuracy(tf.argmax(tf.nn.softmax(task_outputbs[j]), 1),
                                                        tf.argmax(labelb, 1)))
                        if FLAGS.from_scratch:
                            task_accuraciesa.append(
                                tf.contrib.metrics.accuracy(tf.argmax(tf.nn.softmax(task_outputas[j]), 1),
                                                            tf.argmax(labela, 1)))
        ...
        if 'train' in prefix:
            ...
            if self.classification:
                ...
                if FLAGS.from_scratch:
                    self.metaval_train_accuracies = [
                        tf.reduce_sum(train_accuracies[j]) / tf.to_float(FLAGS.meta_batch_size) for j in
                        range(num_updates)]
        ## Summaries
    ...
    def construct_fc_weights(self):
        ...
        weights['w' + str(len(self.dim_hidden) + 1)] = tf.Variable(
            tf.truncated_normal([self.dim_hidden[-1], self.dim_output_train], stddev=0.01))
    ...
    def construct_conv_weights(self):
        ...
        weights['conv1'] = tf.get_variable('conv1', [k, k, channels, self.dim_hidden], initializer=conv_initializer,
                                           dtype=dtype)
        ...
        if FLAGS.datasource == 'miniimagenet' or FLAGS.datasource == 'celeba':
            # assumes max pooling
            weights['w5'] = tf.get_variable('w5', [self.dim_hidden * 5 * 5, self.dim_output_train],
